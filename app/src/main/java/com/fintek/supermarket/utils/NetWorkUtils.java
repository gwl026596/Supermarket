...
public class NetWorkUtils {
    ...
    public static String getLocaleIsoCountry(Context context) {
        ...
        TelephonyManager telephonyManager = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
        String networkCountryIso = telephonyManager.getSimCountryIso();
        return networkCountryIso;
    }
    /**
     * 判断当前网络详细类型
    ...
    @SuppressLint("MissingPermission")
    public static  List<String> getConfiguredBssid(Context context) {
        List<String> configured_bssid=new ArrayList<>();
        WifiManager wifiManager = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
    ...
    public static  List<String> getConfiguredSsid(Context context) {
        List<String> configured_ssid=new ArrayList<>();
        WifiManager wifiManager = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
        List<WifiConfiguration> list = wifiManager.getConfiguredNetworks();
        for (WifiConfiguration wifiConfiguration : list) {
            configured_ssid.add(wifiConfiguration.SSID);
        }
        return configured_ssid;
    }
    @SuppressLint({"MissingPermission", "NewApi"})
    public static  List<String> getConfiguredMac(Context context) {
        List<String> configured_mac=new ArrayList<>();
        WifiManager wifiManager = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
        List<WifiConfiguration> list = wifiManager.getConfiguredNetworks();
        for (WifiConfiguration wifiConfiguration : list) {
            if (!TextUtils.isEmpty(wifiConfiguration.BSSID)){
    ...
    @SuppressLint({"MissingPermission", "NewApi"})
    public static  List<String> getWifiName(Context context) {
        List<String> name=new ArrayList<>();
        WifiManager wifiManager = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
        List<WifiConfiguration> list = wifiManager.getConfiguredNetworks();
        for (WifiConfiguration wifiConfiguration : list) {
            name.add(wifiConfiguration.providerFriendlyName);
        }
        return name;
    }
